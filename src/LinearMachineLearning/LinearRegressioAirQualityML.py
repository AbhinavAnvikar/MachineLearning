...
def main():
    ...
    dat = pd.DataFrame(df[['Time', 'CO(GT)']])
    #dat['Time'] = dat['Time'].replace(to_replace=':00:00', value='', regex=True)

    #dat['Time'] = dat['Time'].astype('int32')
    #data = pd.to_numeric(data)

    print(dat.dtypes)

    x = list(dat['Time'])
    x = [int(str(ch).replace(':00:00','')) for ch in x]
    y = list(dat['CO(GT)'])
    #y = list(map(int,y))
    ...
    m = 0.0
    b = 0.0
    iter = 1000  # no of iteration to train the model
    learn_rate = .001  # Rate of learning
    plt.scatter(x, y,s=10)
    plt.ylabel('CO(GT) Points')
